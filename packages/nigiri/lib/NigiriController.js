NigiriController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('nigiri', this.getCriteria(), sort);
    },
...
{dateAdded: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Nigiri.find(this.getCriteria(), sort);

        return this.render('nigiriIndex', {
            data: {
...
price: t.find('#price').value? Number(t.find('#price').value):null,
dateAdded: t.find('#dateAdded').value? new Date(t.find('#dateAdded').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Nigiri.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', __("insert_success", "") + ".");
                Router.go('nigiriView', {_id: _id});
            });
        }
        return this.render('nigiriInsert', {});
    },
    /* event updating data */
    update: function(t) {
        var _id = this.getId();
        ...
        if (this._post) {
            ...
            Nigiri.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
        ...
        return this.render('nigiriUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Nigiri.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
